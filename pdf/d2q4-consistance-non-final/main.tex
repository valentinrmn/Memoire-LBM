...
\begin{prop}\label{prop:consistance-D2Q4}
    Le $D_2Q_4$ est consistant avec l'équation de la chaleur \eqref{eq:chaleur-2d} si et seulement si $s_1 \notin \{ 0, 2 \}$, $s_2 \neq 0$, $\alpha_i = 0$ pour tout $i \in \{1,\ 2,\ 3\}$ et
    \[
    \frac{\Delta x^2}{\Delta t} := \mu = \frac{4\nu s_1}{2-s_1}%s_1 = \frac{2\mu}{\mu + 4\nu}
    \] 
\end{prop}
...
\] 
qui correspond au Laplacien pondéré par les pas d'espace. En imposant $\Delta x = \Delta y$ et $s_1, s_2 \neq 0$, nous obtenons
\[
...
\begin{align*}
    \frac{1}{4}\left( 1 - \frac{2}{s_1}\right) \frac{\Delta x^2}{\Delta t} = -\nu \iff \mu := \frac{\Delta x^2}{\Delta t} = \frac{4\nu s_1}{2-s_1}%s_1 = \frac{2}{1 + 4 \frac{\nu}{\mu}} = \frac{2\mu}{\mu + 4 \nu}
\end{align*}
...
$\impliedby$ Fixons $\mu = \Delta x^2/\Delta t = \frac{4\nu s_1}{2-s_1}$, $\alpha_i = 0$ pour $i = 1, 2, 3$, $s_1 \notin \{0, 2\}$ et $s_2 \neq 0$. Un calcul direct et similaire à ce qui a été fait précédemment donne $\tau_{i,j}^n = O(\Delta t + \Delta x^2)$.
\end{proof}

\section{Ordre de consistance}

\begin{prop}
Il existe $0<s_2<2$ et $\mu > 0$ qui augmentent l'ordre de consistance du schéma. (conjecture plutôt)
\end{prop}
...
\begin{multline*}
s_1^2s_2 \Delta t \partial_t u =\frac{\Delta x^2}{2} s_2(s_1^2-2s_1) \Delta u + \frac{\Delta t^2}{2}\left( 2s_1^2 + s_1^2 s_2 + 4s_1s_2 \right)\partial_{tt}u + \frac{\Delta x^4}{48} s_2(s_1^2-2s_1) (\partial_{xxxx}u + \partial_{yyyy} u) + \\
\frac{\Delta t \Delta x^2}{4} (2s_1^2 - 4s_1 + s_1^2s_2 - 2s_2) (\partial_{txx} u + \partial_{tyy}u) + \frac{\Delta x^4}{4} (s_1^2 - 4s_1 + 4 + s_1s_2 - 2s_2) \partial_{xxyy} u
\end{multline*}
...
d'où
\begin{multline*}
    s_1^2s_2 \Delta t \partial_t u = \Delta t \frac{\mu \sigma}{2} \Delta u + \frac{\Delta t^2}{2}(2s_1^2 + s_1^2s_2 + 4s_1s_2) \nu^2 (\partial_{xxxx} u + 2 \partial_{xxyy} u + \partial_{yyyy} u ) + \frac{\Delta x^4}{48} s_2 (s_1^2 - 2s_1) ( \partial_{xxxx} u + \partial_{yyyy} u) \\
    + \frac{\Delta t \Delta x^2}{4}(2s_1^2 - 4 s_1 + s_1^2 s_2 - 2s_2) \nu (\partial_{xxxx} u + 2\partial_{xxyy} u + \partial_{yyyy} u ) + \frac{\Delta x^4}{4} (s_1^2 - 4 s_1 + 4 + s_2 s_1 - 2s_2) \partial_{xxyy} u
\end{multline*}
et en écrivant $\Delta t = \Delta x^2/\mu$, on obtient
\begin{multline*}
    s_1^2s_2 \Delta t \partial_t u = \Delta t \frac{\mu \sigma}{2} \Delta u +  \Delta x^4 ( \partial_{xxxx} u + \partial_{yyyy} u ) \biggl(  \frac{1}{2} \frac{\nu^2}{\mu^2}(2s_1^2 + s_1^2 s_2 + 4s_1s_2) + \frac{1}{4}\frac{\nu}{\mu}(2s_1^2 -4s_1 + s_1^2s_2 - 2s_2) + \\\frac{1}{48}s_2(s_1^2-s_1)\biggl) + \Delta x^4 \partial_{xxyy}u \biggl( \frac{\nu^2}{\mu^2}(2s_1^2+s_1^2s_2 + 4s_1s_2) + \frac{1}{2}\frac{\nu}{\mu} (2s_1^2 - 4s_1 + s_1^2s_2 - 2s_2) + \frac{1}{4}(s_1^2 - 4s_1 + 4 + s_1s_2 - 2s_2)\biggl)
\end{multline*}
idée: premier coefficient polynôme en $X = \nu/\mu$ et deuxième coefficient linéaire en $s_2$, possible de s'en sortir? $s_1$ dépend implicitement de $\mu$ donc ça ne marche peut-être pas, quoique polynôme en $X = 1/\mu$?
\end{proof} 
